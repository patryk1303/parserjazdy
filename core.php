<?php
	...
	$rok = "1997 - 2012";
	$sciezka = dirname(__FILE__); //�cie�ka g��wna do plik�w
	$rozklady = $sciezka.'/rozklad'; //�cie�ka do foledu rozk�ad�w
	$rozklady_folder = '/rozklad'; //otwiera folder rozk�ad�w, nieu�ywane
	$linie = getLinie($rozklady);
  
	if (!isset($_GET['linia']))      $_GET['linia']      = 0 ; //je�li linia niewybrana - zmie� warto�� na 0 (false)
	if (!isset($_GET['kierunek']))   $_GET['kierunek']   = 0 ; //je�li kierunek niewybrany - zmie� warto�� na 0 (false)
	if (!isset($_GET['przystanek'])) $_GET['przystanek'] = 0 ; //je�li przystanek niewybrany - zmie� warto�� na 0 (false)
	if (!isset($_GET['lamanie']))    $_GET['lamanie']    = 5; //je�li �amanie niezdefiniowane - zmie� warto�� na 5
	...
	$typy_dni_plik = $rozklady.'/typy_dni'; //otwiera plik typ�w dni - b�dzie to pokazane przy odjazdach
	...
	if ($wLinia != NULL)
	{
		...
		} //zako�czenie po wyborze kierunku
	} //zako�czenie po wyborze linii
	...
	function wybranaLinia() //pokazuje nr wybranej linii, je�li linia niewybrana - pokazuje napis dot. wyboru linii
	{
		global $wLinia;
		if ($wLinia)
	...
	function wyswietlPrzystanki($wybrana_linia, $wybrany_kierunek, $plik_przystanki, $styl=1) //wy�wietla przystanki wybranego kierunku
	{
		global $wPrzystanek;
	...
	function wyswietlOdjazdy($plik_przystanki, $plik_dnia, $kierunek, $typy_dni, $kierunek_plik, $typy_dni_ilosc, $wyswietlKierunekPrzystanek=1, $styl=1) //pokazuje odjazdy wybranego przystanku
	{
		...
		if ($wPrzystanek or $wPrzystanek == 0)
		{
			...
			for ($i=0; $i<$typy_dni_ilosc; $i++)
			{
				$odjazdy[$i] = preg_split("/ /",$plik_dnia[$i][$wPrzystanek]); //przekazuje osobne godziny odjazd�w do macierzy
			}
			
			//ma by� tak: $odjazdy[typ_dnia][godzina][minuta]
			for ($i=0; $i<$typy_dni_ilosc; $i++)
			{
			...
			if ($styl==1)  // wersja normalna
			{
				for ($i=0; $i<$typy_dni_ilosc; $i++)
				{
					...
					if (count($godziny)!=0) echo '<table border="0" class="odjazdy">'; //start tabeli z odjazdami
						if (count($godziny)!=0) echo '<tr><th colspan="'.count($godziny).'"><span class="odjazdyHeader">'.$typy_dni[$i].':</span></th></tr>';	//wy�wietlenie typu dnia
					echo '<tr>';
					if (count($godziny)!=0)
  ...
  function footer()
  {
    ...
    echo "&copy; $firma $rok<br>";
		echo "Dane rozk�ad�w pochoz� ze strony <a href=\"$adres\">$firma</a>";
